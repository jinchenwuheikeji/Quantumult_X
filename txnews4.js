...
const signurlVal = $.getdata('sy_signurl_txnews4')
const cookieVal = $.getdata( 'sy_cookie_txnews4')
const videoVal = $.getdata( 'video_txnews4')
...
if ($request &&$request.body.indexOf("article_read")> -1) {
  ...
  if (signurlVal) $.setdata(signurlVal, 'sy_signurl_txnews4')
  if (cookieVal) $.setdata(cookieVal,  'sy_cookie_txnews4')
  $.msg(cookieName, `获取Cookie: 成功🎉`, ``)
  }
if ($request &&$request.body.indexOf("video_read")> -1) {
  const videoVal =  $request.url
  $.log(`videoVal:${videoVal}`)
  if (videoVal) $.setdata(videoVal,  'video_txnews4')
